...
from AyaMusic import app, SUDOERS
    ...
    for x in served_chats:
        try:
            await app.send_message(
                x,
                f"aya music server has just restarted.\n\nsorry for the issues, start playing after 15-20 seconds again.",
            )
            await remove_active_chat(x)
        except Exception:
            pass
    x = await message.reply_text(f"restarting aya music bot.")
    os.system(f"kill -9 {os.getpid()} && python3 -m AyaMusic")
...
async def update_bot(_, message):
    ...
    if str(m[0]) != "A":
        ...
        return os.system(f"kill -9 {os.getpid()} && python3 -m AyaMusic")
    else:
