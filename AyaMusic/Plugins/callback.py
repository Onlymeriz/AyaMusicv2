...
from AyaMusic.config import LOG_GROUP_ID
from youtubesearchpython import VideosSearch
from ..AyaUtils.tgcallsrun import ASS_ACC
from AyaMusic import app, BOT_USERNAME, dbb, SUDOERS
...
from AyaMusic.AyaUtils.helpers.paste import paste
...
from ..AyaUtils.helpers.gets import (get_url, themes, random_assistant, ass_det)
...
from AyaMusic import aiohttpsession as session
...
async def resumevc(_,CallbackQuery):  
    ...
    if await is_active_chat(chat_id):
        if await is_music_playing(chat_id):
            ...
            await aya.pytgcalls.resume_stream(chat_id)
...
async def skipvc(_,CallbackQuery): 
    ...
    if await is_active_chat(chat_id):
        ...
        if is_empty(CallbackQuery.message.chat.id):
            ...
            await aya.pytgcalls.leave_group_call(chat_id)
            return
        else:
...
▶ personal playlist playing."""
            ...
            for note in _playlist:
                ...
                if await is_active_chat(chat_id):
                    ...
                    await aya.pytgcalls.join_group_call(
                        chat_id,
                        InputStream(
                            InputAudioStream(
                                file,
