from AyaMusic import app, OWNER
import os
import subprocess
import shutil
import re
import sys
import traceback
from AyaMusic.AyaUtils.database.sudo import (get_sudoers, get_sudoers, remove_sudo, add_sudo)
...
async def userdel(_, message: Message):
    if not message.reply_to_message:
        ...
        if removed:
            ...
            return os.execvp("python3", ["python3", "-m", "AyaMusic"])
    ...
    if user_id not in await get_sudoers():
        return await message.reply_text(f"❌ user is not a part of **aya music**")        
